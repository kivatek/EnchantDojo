...
メールを正しく届けるには正しい宛先を指定する必要があります。enchant.jsの場合もタッチの情報を届ける宛先を指定する必要があります。
...
ゲームでよくある例に取るとタイトル画面、ゲーム難易度選択画面、ゲーム本編と切り替わることに相当します。
    ...
    準備中：pushSceneの動きを図解する。スタックの説明図みたいな感じで。
...
enchant.jsではプログラムの途中にScratch「○秒待つ」に相当するプログラムをそのまま書くことはありません。

javascriptのsetTimeout()というメソッドか、またはenchant.jsのタイムラインを工夫してプログラムすることになります。
