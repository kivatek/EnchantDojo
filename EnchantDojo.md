...
また動画で説明されているpushScene以外に関連するメソッドとしてpopScene、replaceScene、removeSceneが用意されています。

動作イメージですが、例えばゲーム開始直後はタイトル画面が表示されているとします。

![alt "図9-1"][lesson09-01]

画面の案内に従ってスクリーンがタッチされたとき、プログラムでタイトル画面に重ねてゲーム本編のシーンをpushSceneします。

動作イメージは次のようになります。内部的にタイトル画面は残っているのですが、目に見えるのはゲーム本編です。

![alt "図9-2"][lesson09-02]

タイトル画面はいわば眠っている状態になるだけです。

ここでゲーム本編をpopSceneするとタイトル画面に戻ります。popSceneはpushSceneに対応する機能です。

    この動きは一例です。シーンを再利用しない場合にはpushSceneではなくreplaceSceneという機能を使う方が適切でしょう。
